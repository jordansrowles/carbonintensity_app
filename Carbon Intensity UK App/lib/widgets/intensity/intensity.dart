...
class IntensityCard extends StatelessWidget {
  ...
  IntensityCard({super.key, required this.intensity}) {
    delta = (intensity.intensity!.forecast! - intensity.intensity!.actual!);
    // format the date to get it looking pretty
    var prettyStart = DateFormat("yyyy MM dd (HH:MM - ")
        .format(DateTime.parse(intensity.from!));
    var prettyEnd = DateFormat("HH:MM)").format(DateTime.parse(intensity.to!));
    date = prettyStart + prettyEnd;
  }
              ...
              Column(
                children: <Widget>[
                  ...
                  Icon(
                    ...
                    size: 50.0,
                    color:
                        getColourFromIntensityIndex(intensity.intensity?.index),
                  ),
                  Text(
                    "${intensity.intensity?.index?.toPascalCase()}",
                    style: Theme.of(context).textTheme.headline6,
                  ),
                ],
              ),
